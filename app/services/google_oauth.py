...
class GoogleOAuthService:
    ...
    async def find_or_create_oauth_user(
        ...
        name = GoogleOAuthService._generate_unique_name(name, email)
        ...
        new_user = User(
            ...
            hashed_password='default'  # OAuth users don't have passwords
        )
    ...
    @staticmethod
    def _generate_unique_name(name: str, email: str) -> str:
        """Generate a unique name from name or email"""
        if name:
            # Clean the name: remove special characters, convert to lowercase
            name = ''.join(c.lower() for c in name if c.isalnum())
            if len(name) >= 3:
                return name[:20]  # Limit to 20 characters
        
        # Fallback to email prefix
        email_prefix = email.split('@')[0]
        name = ''.join(c.lower() for c in email_prefix if c.isalnum())
        return name[:20] if name else f"user{str(uuid.uuid4())[:8]}"
